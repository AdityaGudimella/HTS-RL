...
class RolloutStorage(object):
    ...
    def feed_forward_generator(
        self, advantages, num_mini_batch=None, mini_batch_size=None, is_cen=False
    ):
        ...
        for indices in sampler:
            ...
            if is_cen:
                other_obs_batch = self.other_obs.view(-1, *self.other_obs.size()[2:])[
                    indices
                ]
                other_action_batch = self.other_actions.view(
                    -1, *self.other_actions.size()[2:]
                )[indices]
            if is_aug:
            ...
            if is_cen:
                yield obs_batch, recurrent_hidden_states_batch, actions_batch, value_preds_batch, return_batch, masks_batch, old_action_log_probs_batch, adv_targ, other_obs_batch, other_action_batch
            else:
                if is_aug:
                    yield (
                        obs_batch,
                        aug_obs_batch,
                    ), recurrent_hidden_states_batch, actions_batch, value_preds_batch, return_batch, masks_batch, old_action_log_probs_batch, adv_targ
                else:

...
class RolloutStorageMA(object):
    def __init__(
        ...
        if num_steps is not None:
            ...
            if aug_size > 0:
                self.aug_obs = torch.zeros(
                    num_steps + 1, num_processes, num_agents, aug_size
                )
            else:
                self.aug_obs = None
            ...
            self.actions = torch.zeros(
                num_steps, num_processes, num_agents, action_shape
            )
            if action_space.__class__.__name__ == "Discrete":
    ...
    def single_process_batch_insert(
        self,
        rank,
        obs,
        recurrent_hidden_states,
    ...
    def single_process_insert(
        ...
        self.recurrent_hidden_states[self.step + 1, rank : rank + 1].copy_(
            recurrent_hidden_states
        )
    ...
    def after_update(self, other_storage=None):
        if other_storage is None:
            ...
            self.recurrent_hidden_states[0].copy_(
                other_storage.recurrent_hidden_states[-1]
            )
    ...
    def compute_returns(
        ...
        if use_proper_time_limits:
            ...
            if use_gae:
                ...
                self.returns[-1] = next_value
                masks = torch.stack(
                    [self.masks for _ in range(self.rewards.size(2))], dim=2
                )
                for step in reversed(range(self.rewards.size(0))):
                    self.returns[step] = (
                        self.returns[step + 1] * gamma * masks[step]
                        + self.rewards[step]
                    )

    def feed_forward_generator(
        self, advantages, num_mini_batch=None, mini_batch_size=None, is_aug=False
    ):
        ...
        for indices in sampler:
            ...
            old_action_log_probs_batch = self.action_log_probs.view(
                -1, *self.action_log_probs.size()[2:]
            )[indices]
            if is_aug:
                aug_obs_batch = self.aug_obs[:-1].view(-1, *self.aug_obs.size()[2:])[
                    indices
                ]
            if advantages is None:
                adv_targ = None
            else:
                adv_targ = advantages.view(-1, *advantages.size()[2:])[indices].view(
                    -1, *advantages.size()[3:]
                )

            if is_aug:
                yield obs_batch.view(-1, *obs_batch.size()[2:]), aug_obs_batch.view(
                    -1, *aug_obs_batch.size()[2:]
                ), recurrent_hidden_states_batch, actions_batch.view(
                    -1, 1
                ), value_preds_batch.view(
                    -1, 1
                ), return_batch.view(
                    -1, 1
                ), masks_batch, old_action_log_probs_batch.view(
                    -1, 1
                ), adv_targ.view(
                    -1, 1
                )
    ...
    def get_one_agent_rollout(self, agent_idx, is_cen=False, is_aug=False):
        ...
        if is_cen:
            ...
            if agent_idx != 0 and agent_idx != self.num_agents - 1:
                obs_layer4_1 = torch.narrow(
                    self.obs_layer4, dim=2, start=0, length=agent_idx
                )
                obs_layer4_2 = torch.narrow(
                    self.obs_layer4,
                    dim=2,
                    start=agent_idx + 1,

...
class TestDeterministic(unittest.TestCase):
    ...
    def test_deter(self):
        for actor_rank in range(self.num_actors):
            self.assertEqual(
                self.prev_traj_runs0[actor_rank]["action"],
                self.prev_traj_runs1[actor_rank]["action"],
            )
        for actor_rank in range(self.num_actors):
            self.assertEqual(
                self.prev_traj_runs0[actor_rank]["v"],
                self.prev_traj_runs1[actor_rank]["v"],
            )
        for actor_rank in range(self.num_actors):
            for step in range(self.n_step):
                self.assertTrue(
                    np.array_equal(
                        self.prev_traj_runs0[actor_rank]["obs"][step],
                        self.prev_traj_runs1[actor_rank]["obs"][step],
                    )
                )
        for actor_rank in range(self.num_actors):
            for step in range(self.n_step):
                self.assertTrue(
                    np.array_equal(
                        self.prev_traj_runs0[actor_rank]["action_logit"][step],
                        self.prev_traj_runs1[actor_rank]["action_logit"][step],
                    )
                )

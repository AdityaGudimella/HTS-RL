...
def env_step(
    ...
    local_rollouts = RolloutStorageMA(
        args.sync_every,
        1,
    ...
    for step in range(num_steps):
        ...
        obs, reward, done, infos = env.step(action.item())
        if args.dump_traj_flag:
            dump_traj["action"].append(action)
            dump_traj["obs"].append(prev_obs)
            dump_traj["action_logit"].append(copy.deepcopy(action_logits[rank]))
        ...
        bad_masks = (
            ...
            if "bad_transition" in infos.keys()
            else torch.FloatTensor([1.0])
        )
        ...
        if step % args.sync_every == 0 and step != 0:
            ...
            if sync_count == 19 and args.dump_traj_flag:
                ...
                with open(
                    "/tmp/traj_run{}_{}.pkl".format(args.dump_run_id, rank), "wb"
                ) as output:
            ...
            if rank == 0 and sync_count % 100 == 0:
                ...
                plot["final_rewards"].append(np.mean(plot["avg_rewards"][-10:]))
